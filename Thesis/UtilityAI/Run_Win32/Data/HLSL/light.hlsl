...
float3 ColorToSurfaceNormal(float3 n)
{
    float3 normal = n * float3(2, 2, 1) - float3(1, 1, 0);
...
float4 FragmentFunction( vertex_to_fragment_t data ) : SV_Target0
{
    ...
    float3 bitangent = data.bitangent;//normalize(cross(tangent, normal));
    //tangent = normalize(cross(normal, bitangent));

    //return float4(data.uv, 0.f, 1.f);
    //return float4((normal + 1) * 0.5f, 1.f);
    //return float4((tangent + 1) * 0.5f, 1.f);
    //return float4((bitangent + 1) * 0.5f, 1.f);
    ...
    float3 snormal = ColorToSurfaceNormal(normal_color);
    snormal = mul(snormal, tbn);
    ...
    float4 light_diffuse_factor = 0.0f;
    float4 spec_color = 0.0f;
    for (int light_index = 0; light_index < 8; light_index++)
    {
        ...
        float3 vector_to_light = LIGHTS[light_index].LIGHT_POSITION.xyz - data.world_position;
        ...
        float attenuation = LIGHTS[light_index].LIGHT_COLOR.w / (LIGHTS[light_index].ATTENUATION.x
            + distance_to_light * LIGHTS[light_index].ATTENUATION.y
            + distance_to_light * distance_to_light * LIGHTS[light_index].ATTENUATION.z);

        // don't allow attenuation to go above one (so don't allow it to blow out the image)
        attenuation = saturate(attenuation);

        float4 light_color = float4(LIGHTS[light_index].LIGHT_COLOR.xyz, 1.0f);
        light_diffuse_factor += light_color * dot3 * attenuation;

        // Calculate Spec Component
        float spec_attenuation = LIGHTS[light_index].LIGHT_COLOR.w / (LIGHTS[light_index].SPEC_ATTENUATION.x
            + distance_to_light * LIGHTS[light_index].SPEC_ATTENUATION.y
            + distance_to_light * distance_to_light * LIGHTS[light_index].SPEC_ATTENUATION.z);
